...
class AccessToken:
    ...
    def build(self):
        ...
        val = self.appID + self.channelName + self.uidStr + f"{m}"
        signature = hmac.new(codecs.encode(self.appCertificate), codecs.encode(val), sha256).digest()
        crc_channel_name = crc32(codecs.encode(self.channelName)) & 0xffffffff
        crc_uid = crc32(codecs.encode(self.uidStr)) & 0xffffffff
        ...
        version = getVersion()
        ret = version + self.appID + f"{base64.b64encode(content)}"
        return ret
